...
ResultadoPrim* Menu::Prim(GRAFO* grafo) {
  ...
  int numVertices = 0;
  for (Vertice* v = grafo->PrimerVertice(); v != nullptr; v = grafo->SiguienteVertice(v)) {
  ...
  std::priority_queue<std::pair<double, Vertice*>, std::vector<std::pair<double, Vertice*>>, std::greater<std::pair<double, Vertice*>>> cola;
  ...
  while (!cola.empty()) {
    ...
    if (visitado[indice[vertice]]) continue;
    visitado[indice[vertice]] = true;

    for (Vertice* adyacente = grafo->PrimerVerticeAdyacente(vertice); adyacente != nullptr; adyacente = grafo->SiguienteVerticeAdyacente(vertice, adyacente)) {
